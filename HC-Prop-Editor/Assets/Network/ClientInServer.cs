...
public class ClientInServer
{
    ...
    public ClientInServer(int _clientId)
    {
        id = _clientId;
        tcp = new TCP(id);
        udp = new UDP(id);
    ...
    public class TCP
    {
        ...
        {
            Debug.Log($"Tcp client ip, port: {_socket.Client.RemoteEndPoint.ToString()}");
            socket = _socket;
        ...
        private bool HandleData(byte[] _data)
        {
            ...
            if (receivedData.UnreadLength() >= 4)
            {
                ...
                // Debug.Log("the length = " + _packetLength);
                if (_packetLength <= 0)
                {
            ...
            while (_packetLength > 0 && _packetLength <= receivedData.UnreadLength())
            {
                ...
                ThreadManager.ExecuteOnMainThread(() =>
                {
                    using (Packet _packet = new Packet(_packetBytes))
                    {
                        ...
                        Debug.Log("the packet id = " + _packetId);
